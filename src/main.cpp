...
float flight_loop(float inElapsedSinceLastCall,
                  ...
                  void *inRefcon);
float flight_loop_kdt(float inElapsedSinceLastCall,
                      float inElapsedTimeSinceLastFlightLoop, int inCounter,
                      void *inRefcon);
...
static int DrawCallback1(XPLMDrawingPhase inPhase, int inIsBefore,
                         void *inRefcon) {
  ...
  if (!printed) {
    ...
    for (int i = 0; i < 4; i++) {
      sprintf(buffer, "m_wrl %.2f %.2f %.2f %.2f\n", mv[0 + i * 4],
              mv[1 + i * 4], mv[2 + i * 4], mv[3 + i * 4]);
      lg.xplm(std::string(buffer));
    }
    for (int i = 0; i < 4; i++) {
      sprintf(buffer, "proj  %.2f %.2f %.2f %.2f\n", proj[0 + i * 4],
              proj[1 + i * 4], proj[2 + i * 4], proj[3 + i * 4]);
      lg.xplm(std::string(buffer));
    }
    /*
  ...
  for (auto i = 1; i < ai;
    ...
    if (acf_ndc[3] >= 0) { // < 0 means behind me
      ...
      float dist =
          sqrt(dx * dx +
               dz * dz); // there is a tcas / relative_distance_mtrs dataref
      ...
      if (dist < 100 * 1000) { // only < 100 kms
        ...
        // sim/cockpit2/tcas/indicators/relative_distance_mtrs     float[64] y
        // meters  Distance to each other
        // sim/cockpit2/tcas/indicators/relative_altitude_mtrs     float[64] y
        // meters  Relative altitude
        // sim/cockpit2/tcas/targets/position/vertical_speed       float[64] y
        // feet/min sim/cockpit2/tcas/targets/position/V_msc float[64]       n
        // meter/s total true speed sim/cockpit2/tcas/targets/position/ele
        // float[64]	n	  meter	global coordinate

        float colWHT[] = {1.0, 1.0, 1.0};
        make_dist_str(dist, dist_buf, units);
        make_spd_str(v_msc, spd_buf, units);
        make_alt_str(ele, alt_buf, units);
        // For compatibility with A-B-C, we use    i+1 (user's AC is 1, not
        // zero)
        sprintf(buffer, "%i | %s | %s | %s | %c", i + 1, dist_buf, spd_buf,
                alt_buf, v_spd);
        // lg.xplm( std::string(buffer)+"\n" );
        ...
        XPLMDrawTranslucentDarkBox(box_x - 5, box_y + 10, box_x + 6 * len + 5,
                                   box_y - 8); // slow?
        XPLMDrawString(colWHT, box_x, box_y - 1, buffer, NULL, xplmFont_Basic);
        // XPLMDrawString(colWHT, box_x, box_y-1, buffer, NULL,
        // xplmFont_Proportional);

        // draw the label higer, and a small green line to the final_x and
        // final_y points?
        if (false) {
...
// The KDT POIs.
static int DrawCallback_kdt(XPLMDrawingPhase inPhase, int inIsBefore,
                            void *inRefcon) {
  ...
  for (auto &poi : pois) { // break when max_shown are displayed
    ...
    poi_to_local(plat, plon, poi_x, poi_y,
                 poi_z); // only needed on scenery switch?!
    poi.x = poi_x;
    poi.y = poi_y;
    poi_y += alt;
    poi.z = poi_z;
    ...
    double latlon_dist =
        dist_latlon(uplat, uplon, plat, plon); // but not exact.... hmmm
    ...
    */
    ...
    float acf_wrl[4] = {(float)poi_x, (float)poi_y, (float)poi_z, 1.0f};
...
static void warp_forwards() {
  ...
  // adjust agl
  // ground_y = dr_plugin_ground_center.get_float(1); // diff from first
  // measurement? dr_pos_y.set_float( ground_y + diff_y );
...
static void warp_to_closest_ai() {
  ...
  */
  ...
  // Or just step through the list! Slower, but it is a short delay
  // and we are warping the aircraft anyway.
  int closest_idx = 1;
  ...
  lg.xplm("warp_to_next_ai():ai x/y/z vel = " + std::to_string(vx1) + ", " +
          std::to_string(vy1) + ", " + std::to_string(vz1) + ", " + "\n");
#endif
  ...
  // because we need to put the vx/vz right, reclculate it for the new headingm
  // aybe
  angle_offset = psi1; // just swap s and c instead?
  s = sin(angle_offset * (double)(M_PI / 180));
  c = -cos(angle_offset * (double)(M_PI / 180));
  float user_vel = sqrt((dr_vel_x.get_float() * dr_vel_x.get_float()) +
                        (dr_vel_z.get_float() * dr_vel_z.get_float()));
...
static void warp_to_prev_ai() {
  ...
  int i = closest_idx;
  float lx1 =
      static_cast<float>(dr_tcas_pos_x.get_memory(i)); // or were these doubles
  float lz1 = static_cast<float>(dr_tcas_pos_z.get_memory(i));
  float ly1 = static_cast<float>(dr_tcas_pos_y.get_memory(i));
  float ele1 = static_cast<float>(dr_tcas_pos_ele.get_memory(i));

  float vx1 =
      static_cast<float>(dr_tcas_vel_x.get_memory(i)); // or were these doubles
  float vz1 = static_cast<float>(dr_tcas_vel_z.get_memory(i));
  float vy1 = static_cast<float>(dr_tcas_vel_y.get_memory(i));
  ...
  float phi1 = static_cast<float>(
      dr_tcas_pos_phi.get_memory(i)); // or were these doubles
  float psi1 = static_cast<float>(dr_tcas_pos_psi.get_memory(i)); // heading
  float the1 = static_cast<float>(dr_tcas_pos_the.get_memory(i));
...
static void smoke_airports() {
  ...
  for (auto &poi : pois) {
    ...
    if (latlon_dist >= max_poi_dist) { // they can have different distances, so a short one
      // disables a longer one after it. maybe sort on diff
      // between distance and viewdistance?
      // break;//continue; // skip if too far away // break; // we are sorted
      continue;
    }

    Smoker *s = new Smoker();
    if (s->load_obj(pss_obj->path)) {
      ; // copy from the global one
...
int toggle_ac_label_handler(XPLMCommandRef inCommand, XPLMCommandPhase inPhase,
                            void *inRefcon) {
...
int toggle_ap_label_handler(XPLMCommandRef inCommand, XPLMCommandPhase inPhase,
                            void *inRefcon) {
  if (inPhase == xplm_CommandBegin) { // xplm_CommandContinue (1), xplm_CommandEnd (2)
...
int toggle_ap_smoker_handler(XPLMCommandRef inCommand, XPLMCommandPhase inPhase,
                             void *inRefcon) {
  if (inPhase == xplm_CommandBegin) { // xplm_CommandContinue (1), xplm_CommandEnd (2)
    smoke_airports();
  }
...
int toggle_ua_smoke_handler(XPLMCommandRef inCommand, XPLMCommandPhase inPhase,
                            void *inRefcon) {
  if (inPhase == xplm_CommandBegin) { // xplm_CommandContinue (1), xplm_CommandEnd (2)
...
int toggle_units_handler(XPLMCommandRef inCommand, XPLMCommandPhase inPhase,
                         void *inRefcon) {
  if (inPhase == xplm_CommandBegin) { // xplm_CommandContinue (1), xplm_CommandEnd (2)
    units = (++units % 3);
  }
...
int toggle_warp_to_next_ai_handler(XPLMCommandRef inCommand,
...
int toggle_warp_to_prev_ai_handler(XPLMCommandRef inCommand,
                                   XPLMCommandPhase inPhase, void *inRefcon) {
  if (inPhase == xplm_CommandBegin) { // xplm_CommandContinue (1), xplm_CommandEnd (2)
    warp_to_prev_ai();
  }
...
int toggle_warp_to_closest_ai_handler(XPLMCommandRef inCommand,
...
int toggle_warp_forwards_handler(XPLMCommandRef inCommand,
                                 XPLMCommandPhase inPhase, void *inRefcon) {
  if (inPhase == xplm_CommandBegin) { // xplm_CommandContinue (1), xplm_CommandEnd (2)
    warp_forwards();
  }
...
int max_shown_inc_handler(XPLMCommandRef inCommand, XPLMCommandPhase inPhase,
                          void *inRefcon) {
  if (inPhase == xplm_CommandBegin) { // xplm_CommandContinue (1), xplm_CommandEnd (2)
    if (max_shown < 128) {
      max_shown += 1;
...
int max_shown_dec_handler(XPLMCommandRef inCommand, XPLMCommandPhase inPhase,
                          void *inRefcon) {
  if (inPhase == xplm_CommandBegin) { // xplm_CommandContinue (1), xplm_CommandEnd (2)
    if (max_shown > 1) {
      max_shown -= 1;
...
int max_dist_inc_handler(XPLMCommandRef inCommand, XPLMCommandPhase inPhase,
                         void *inRefcon) {
  if (inPhase == xplm_CommandBegin) { // xplm_CommandContinue (1), xplm_CommandEnd (2)
...
int max_dist_dec_handler(XPLMCommandRef inCommand, XPLMCommandPhase inPhase,
                         void *inRefcon) {
  if (inPhase == xplm_CommandBegin) { // xplm_CommandContinue (1), xplm_CommandEnd (2)
...
//    Here we only know X and Y; we’ll leave Z = 0 (or whatever altitude you
prefer) MyPoi query(0, 0, 0, 0, "", ""); query.x = myX;   // your X‐coordinate
...
*/
  ...
  for (auto i = 0; i < ai; i++) {
    ...
    lg.xplm(" POS AI x/y/z " + std::to_string(lx) + ", " + std::to_string(ly) +
            ", " + std::to_string(lz) + "\n");
  }
  ...
  toggle_ac_label_cmd =
      XPLMCreateCommand("durian/xlabel/toggle_ac_label", "Toggle a/c label");
  XPLMRegisterCommandHandler(toggle_ac_label_cmd, toggle_ac_label_handler, 0,
                             (void *)0);

  toggle_ap_label_cmd = XPLMCreateCommand("durian/xlabel/toggle_ap_label",
                                          "Toggle airport/poi label");
  XPLMRegisterCommandHandler(toggle_ap_label_cmd, toggle_ap_label_handler, 0,
                             (void *)0);

  toggle_ap_smoker_cmd = XPLMCreateCommand("durian/xlabel/toggle_ap_smoker",
                                           "Toggle airport/poi smoker");
  XPLMRegisterCommandHandler(toggle_ap_smoker_cmd, toggle_ap_smoker_handler, 0,
                             (void *)0);

  toggle_ua_smoke_cmd = XPLMCreateCommand("durian/xlabel/toggle_ua_smoke",
                                          "Toggle user aircraft smoke");
  XPLMRegisterCommandHandler(toggle_ua_smoke_cmd, toggle_ua_smoke_handler, 0,
                             (void *)0);

  toggle_units_cmd =
      XPLMCreateCommand("durian/xlabel/toggle_units", "Toggle meters/feet");
  XPLMRegisterCommandHandler(toggle_units_cmd, toggle_units_handler, 0,
                             (void *)0);

  toggle_warp_to_next_ai_cmd = XPLMCreateCommand(
      "durian/xlabel/warp_to_next_ai", "Warp user aicraft to next AI aircraft");
  XPLMRegisterCommandHandler(toggle_warp_to_next_ai_cmd,
                             toggle_warp_to_next_ai_handler, 0, (void *)0);

  toggle_warp_to_prev_ai_cmd =
      XPLMCreateCommand("durian/xlabel/warp_to_prev_ai",
                        "Warp user aircraft to previous AI aircraft");
  XPLMRegisterCommandHandler(toggle_warp_to_prev_ai_cmd,
                             toggle_warp_to_prev_ai_handler, 0, (void *)0);

  toggle_warp_to_closest_ai_cmd =
      XPLMCreateCommand("durian/xlabel/warp_to_closest_ai",
                        "Warp user aircraft to closest AI aircraft");
  XPLMRegisterCommandHandler(toggle_warp_to_closest_ai_cmd,
                             toggle_warp_to_closest_ai_handler, 0, (void *)0);

  toggle_warp_forwards_cmd =
      XPLMCreateCommand("durian/xlabel/warp_forwards", "Warp user forwards");
  XPLMRegisterCommandHandler(toggle_warp_forwards_cmd,
                             toggle_warp_forwards_handler, 0, (void *)0);

  max_shown_inc_cmd =
      XPLMCreateCommand("durian/xlabel/max_shown_inc", "Show more labels");
  XPLMRegisterCommandHandler(max_shown_inc_cmd, max_shown_inc_handler, 0,
                             (void *)0);

  max_shown_dec_cmd =
      XPLMCreateCommand("durian/xlabel/max_shown_dec", "Show less labels");
  XPLMRegisterCommandHandler(max_shown_dec_cmd, max_shown_dec_handler, 0,
                             (void *)0);

  max_dist_inc_cmd = XPLMCreateCommand("durian/xlabel/max_dist_inc",
                                       "Increase label viz distance");
  XPLMRegisterCommandHandler(max_dist_inc_cmd, max_dist_inc_handler, 0,
                             (void *)0);

  max_dist_dec_cmd = XPLMCreateCommand("durian/xlabel/max_dist_dec",
                                       "Decrease label viz distance");
  XPLMRegisterCommandHandler(max_dist_dec_cmd, max_dist_dec_handler, 0,
                             (void *)0);
  ...
  XPLMRegisterDrawCallback(DrawCallback_kdt, xplm_Phase_Window, 0,
                           NULL); // slow
...
PLUGIN_API void XPluginStop(void) {
  ...
  XPLMUnregisterCommandHandler(toggle_ac_label_cmd, toggle_ac_label_handler, 0,
                               (void *)0);
  XPLMUnregisterCommandHandler(toggle_ap_label_cmd, toggle_ap_label_handler, 0,
                               (void *)0);
  XPLMUnregisterCommandHandler(toggle_ap_smoker_cmd, toggle_ap_smoker_handler,
                               0, (void *)0);
  XPLMUnregisterCommandHandler(toggle_ua_smoke_cmd, toggle_ua_smoke_handler, 0,
                               (void *)0);
  XPLMUnregisterCommandHandler(toggle_units_cmd, toggle_units_handler, 0,
                               (void *)0);
  XPLMUnregisterCommandHandler(toggle_warp_to_next_ai_cmd,
                               toggle_warp_to_next_ai_handler, 0, (void *)0);
  XPLMUnregisterCommandHandler(toggle_warp_to_prev_ai_cmd,
                               toggle_warp_to_prev_ai_handler, 0, (void *)0);
  XPLMUnregisterCommandHandler(toggle_warp_to_closest_ai_cmd,
                               toggle_warp_to_closest_ai_handler, 0, (void *)0);
  XPLMUnregisterCommandHandler(toggle_warp_forwards_cmd,
                               toggle_warp_forwards_handler, 0, (void *)0);
  XPLMUnregisterCommandHandler(max_shown_inc_cmd, max_shown_inc_handler, 0,
                               (void *)0);
  XPLMUnregisterCommandHandler(max_shown_dec_cmd, max_shown_dec_handler, 0,
                               (void *)0);
  XPLMUnregisterCommandHandler(max_dist_inc_cmd, max_dist_inc_handler, 0,
                               (void *)0);
  XPLMUnregisterCommandHandler(max_dist_dec_cmd, max_dist_dec_handler, 0,
                               (void *)0);
...
PLUGIN_API int XPluginEnable(void) {
  ...
  for ( auto i = 0; i < ai; i++ ) {
    ...
    double lz  = static_cast<double>(dr_tcas_pos_z.get_memory(i));
    double ly  = static_cast<double>(dr_tcas_pos_y.get_memory(i));
    lg.xplm( " POS AI x/y/z "+std::to_string(i)+": "+std::to_string(lx)+",
...
PLUGIN_API void XPluginReceiveMessage(XPLMPluginID inFrom, int inMsg,
  ...
  if (inMsg == XPLM_MSG_PLANE_LOADED) {
    ...
    for ( auto& poi : pois) {
      ...
      poi_to_local(plat, plon, poi_x, poi_y, poi_z); // only needed on scenery
    switch?! poi.x = poi_x; poi.y = poi_y; poi_y += alt; poi.z = poi_z;
      poi.update = 0;
    }
    */
    ...
    std::sort(begin(pois), end(pois), [p](const poi &lhs, const poi &rhs) {
      return dist_latlon(p.lat, p.lon, lhs.lat, lhs.lon) <
             dist_latlon(p.lat, p.lon, rhs.lat, rhs.lon);
    });
...
// WHat this should do is to fill the poi vector with the nearest, the
// DrawCallBack function will draw the first 10 of the poi vector.
float flight_loop_kdt(float inElapsedSinceLastCall,
                      float inElapsedTimeSinceLastFlightLoop, int inCounter,
                      void *inRefcon) {
